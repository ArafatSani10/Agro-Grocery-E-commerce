...
export default function Modal({ isOpen, closeModal, data }) {
  ...
  // children[] comes from the new relational Category model
  const subCategories = Array.isArray(data.parentCategory?.children)
                  ...
                  <div className="md:w-[55%] p-8 md:pl-0 flex flex-col justify-center">
                    <div className="space-y-4">
                      <div>
                        ...
                        <div className="mt-1">
                          {isOutOfStock ? (
                            <span className="text-[10px] bg-red-50 text-red-600 px-2.5 py-0.5 rounded font-bold border border-red-100">
                              Stock Out
                            </span>
                          ) : (
                            <span className="text-[10px] bg-emerald-50 text-emerald-600 px-2.5 py-0.5 rounded font-bold border border-emerald-100">
                              In Stock
                            </span>
                          )}
                        </div>
                      ...
                      <div className="flex items-center gap-3 pt-2">
                        <div
                          className={`flex items-center border border-gray-200 rounded h-10 bg-white ${isOutOfStock ? "opacity-40 cursor-not-allowed" : ""}`}
                        >
                          <button
                            type="button"
